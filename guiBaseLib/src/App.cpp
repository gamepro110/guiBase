...
namespace guiBase {
    ...
    App::App(WindowSettings windowSettings) : settings(windowSettings) {
        init();
    }
    ...
    void App::init() {
        ...
        windowHandle = glfwCreateWindow(settings.width, settings.height, settings.name.c_str(), nullptr, nullptr);
    ...
    void App::run() {
        ...
        while (!glfwWindowShouldClose(windowHandle) && gRunning) {
...

            {
                ...
                // otherwise any change of dockspace/settings would lead to windows being stuck in limbo and never being
                // visible.
